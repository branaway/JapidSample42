...
public class Application extends JapidController {
	public static Result index() {
		return renderJapid(); // use the default index.html in the
								// japidviews/SampleController directory
		// renderJapidWith("@index.html"); // use the default index.html in the
		// japidviews/SampleController directory
	}
	...
	// };
	//
	// // throw new JapidResult(r.run()).eval(); // eval effectively cancel
	// nested finer cache control
	// render(r);
	// }
	//
	// @CacheFor("6s")
	...
	public static Result testCacheForEager(String p) {
		...
		String b = "" + new Date().getSeconds();
		return null;
		// renderJapidEager(b); // no nested cache control. the outer cache
		// control overrides all
	}
	...
	/**
	 * this method shows how to render arguments to a japid template by naming
	 * and positional convention with the renderJapid().
	 * 
	 */
	...
	/**
	 * demo how to composite a page with independent segments with the #{invoke
	 * } tag
	 */
	...
	/**
	 * "official" Play treats body as a special param name to store all POST
	 * body if the content type is application/x-www-form-urlencoded. bran's
	 * fork has changed the reserved param name to _body.
	 * 
	 * @param f1
	 * @param f2
	 * @param body
	...
	public static Result dumpPost2() {
		...
		if (dumpForm.hasErrors()) {
			return badRequest("authenticity validation FAILED. This request was forged.");
		} else {
			flash("msg", "the form is authentic!");
			Dump d = dumpForm.get();
			return redirect(routes.Application.dumpPost(d.f1, d.f2, d.body));
		}
	}
	...
	public static Result categories() {
		...
		a.name = "a name cool name";
		Category cate1 = new Category();
		cate1.name = "1";
		cate1.attr = "some attr";
